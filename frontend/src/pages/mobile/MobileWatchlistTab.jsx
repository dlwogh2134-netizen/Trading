...
function WatchlistCandlestickChart({ item, assetType, cryptoChartMode, onCryptoChartModeChange, onLatestPriceChange, compact = false }) {
  ...
  return (
    <div className={`rounded-lg border border-[#1f2945]/60 bg-[#0e1529] ${compact ? 'p-2.5' : 'p-3'}`}>
      <div className={`${compact ? 'mb-2' : 'mb-3'} grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2`}>
        ...
        <div className="col-span-2 ml-auto flex min-w-0 flex-wrap justify-end gap-1 md:col-span-1">
          ...
          <div className="flex min-w-0 flex-wrap justify-end gap-1 rounded border border-[#2b395b] bg-[#1b253b] p-0.5">
            {intervalOptions.map((option) => (
              <button
                key={option.value}
                className={`rounded ${compact ? 'px-1.5 py-0.5' : 'px-2 py-1'} text-[10px] font-bold transition ${chartInterval === option.value ? 'bg-cyan-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}
                type="button"
      ...
      <div className="relative overflow-hidden rounded bg-[#0e1529]" style={{ minHeight: chartHeight }}>
        {loadingChart ? (
...
export default function WatchlistTab({ displayCurrency = 'KRW', exchangeRate = 1380, mobileLayout = false }) {
  ...
  function handleRefreshNews() {
    if (!selectedItem || newsLoading) return
    setNewsSyncMessage({ text: '', isError: false })
    void loadWatchlistNewsForItem(selectedItem)
  }
  ...
  return (
    <main className="max-w-7xl mx-auto flex flex-col gap-6">
      ...
      <section className="bg-slate-surface border border-slate-700/80 rounded-lg p-5">
        ...
        <div className={`${mobileLayout ? 'mt-3 grid-cols-2 gap-2' : 'mt-4 gap-3 md:grid-cols-4'} grid`}>
      ...
      <section className="bg-slate-surface border border-slate-700/80 rounded-lg p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={newsLoading || !selectedItem}
            className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-500/40 bg-cyan-950/30 px-3 py-2 text-[11px] font-bold text-cyan-300 transition hover:bg-cyan-900/40 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <svg
              aria-hidden="true"
              className={`h-3.5 w-3.5 shrink-0 ${newsLoading ? 'animate-spin' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth="2"
