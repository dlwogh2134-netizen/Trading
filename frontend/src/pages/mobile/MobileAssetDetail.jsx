...
import AssetDetailMlSignalPanel from '../assetDetailMlSignalPanel.jsx'
import AssetDetailNewsDisclosurePanel from '../assetDetailNewsDisclosurePanel.jsx'
import { AssetDetailOpenOrdersPanel, AssetDetailPositionSummary } from '../assetDetailOrderPanels.jsx'
import {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className={`${showLeftDetailColumn ? 'flex' : 'hidden'} ${showLevel2Panel ? 'lg:col-span-6' : 'lg:col-span-12'} flex-col gap-5`}>
            ...
            <AssetDetailPositionSummary
              holdingSummaryLabel={holdingSummaryLabel}
              availableCashLabel={availableCashLabel}
              openOrdersCount={openOrders.length}
              className="hidden grid-cols-1 gap-3 sm:grid-cols-3"
            />

            <AssetDetailOpenOrdersPanel
              openOrders={openOrders}
              openOrdersLoading={openOrdersLoading}
              orderActionLoadingId={orderActionLoadingId}
              orderManagementMessage={orderManagementMessage}
              brokerEnv={brokerEnv}
              modifyOrderId={modifyOrderId}
              modifyDraft={modifyDraft}
              formatUnitPrice={formatUnitPrice}
              onSyncOpenOrders={handleSyncOpenOrders}
              onOpenModifyOrder={handleOpenModifyOrder}
              onCancelOpenOrder={handleCancelOpenOrder}
              onSubmitModifyOrder={handleSubmitModifyOrder}
              onModifyDraftChange={setModifyDraft}
              onCloseModifyOrder={() => {
                setModifyOrderId('')
                setModifyDraft({ price: '', quantity: '' })
              }}
              className="hidden rounded-xl border border-[#1f2945] bg-[#0e1529]/90 p-4 backdrop-blur-md"
            />

            <AssetDetailAutoRulesPanel
              className="hidden rounded-xl border border-[#1f2945] bg-[#0e1529]/90 p-4 backdrop-blur-md"
              showAddRuleForm={showAddRuleForm}
              myHolding={myHolding}
              currentPrice={currentPrice}
              addRulePrice={addRulePrice}
              setAddRulePrice={setAddRulePrice}
              addRuleQty={addRuleQty}
              setAddRuleQty={setAddRuleQty}
              addRuleProfitRate={addRuleProfitRate}
              setAddRuleProfitRate={setAddRuleProfitRate}
              addRuleStopRate={addRuleStopRate}
              setAddRuleStopRate={setAddRuleStopRate}
              addRuleAutoRestart={addRuleAutoRestart}
              setAddRuleAutoRestart={setAddRuleAutoRestart}
              addRuleExecutionMode={addRuleExecutionMode}
              setAddRuleExecutionMode={setAddRuleExecutionMode}
              autoRulesMessage={autoRulesMessage}
              autoRulesLoading={autoRulesLoading}
              autoRules={autoRules}
              ruleUpdating={ruleUpdating}
              editingRuleId={editingRuleId}
              editTargetProfit={editTargetProfit}
              setEditTargetProfit={setEditTargetProfit}
              ...
              oppositeCurrentPrice={oppositeCurrentPrice}
              formatUnitPrice={formatUnitPrice}
              getCurrencySign={getCurrencySign}
              getCurrencyDigits={getCurrencyDigits}
              onToggleAddRuleForm={setShowAddRuleForm}
              onCloseAddRuleForm={() => setShowAddRuleForm(false)}
              onRefreshAutoRules={loadAutoTradingRules}
              onAddRule={handleAddRule}
              onStartEditRule={handleStartEditRule}
              onCloseEditRule={() => setEditingRuleId(null)}
            ...
            <div className="bg-[#0e1529]/90 border border-[#1f2945] rounded-xl p-5 backdrop-blur-md">
              ...
              <MobileAssetNewsDisclosureSection>
                <AssetDetailNewsDisclosurePanel
                  activeTab={activeTab}
                  isNewsDisclosureSectionActive={isNewsDisclosureSectionActive}
                  newsList={newsList}
                  loadingNews={loadingNews}
                  summaryLoadingId={summaryLoadingId}
                  selectedNewsId={selectedNewsId}
                  newsSyncing={newsSyncing}
                  newsSyncMessage={newsSyncMessage}
                  disclosureList={disclosureList}
                  loadingDisclosures={loadingDisclosures}
                  disclosureAnalyses={disclosureAnalyses}
                  selectedDisclosureId={selectedDisclosureId}
                  disclosureAnalysisLoadingId={disclosureAnalysisLoadingId}
                  disclosureSyncing={disclosureSyncing}
                  disclosureSyncMessage={disclosureSyncMessage}
                  onToggleNewsSummary={handleToggleNewsSummary}
              ...
              <MobileAssetCommunitySection>
                <AssetDetailCommunityPanel
                  ...
                  communityCurrentUserId={communityCurrentUserId}
                  communityDraft={communityDraft}
                  setCommunityDraft={setCommunityDraft}
                  communityReplyParentId={communityReplyParentId}
                  setCommunityReplyParentId={setCommunityReplyParentId}
                  communityReplyDraft={communityReplyDraft}
                  setCommunityReplyDraft={setCommunityReplyDraft}
                  communityLoading={communityLoading}
                  communitySubmitting={communitySubmitting}
                  communityActionId={communityActionId}
                  communityMessage={communityMessage}
                  communityRootPosts={communityRootPosts}
                  communityReplyMap={communityReplyMap}
                  onSubmitCommunityPost={handleSubmitCommunityPost}
                  onSubmitCommunityReply={handleSubmitCommunityReply}
                  onUpdateCommunityStatus={handleUpdateCommunityStatus}
                />
              </MobileAssetCommunitySection>
            </div>
            ) : null}
          ...
          <div className={`${isOrderSectionActive ? 'flex' : 'hidden'} lg:col-span-12 flex-col gap-5`}>
            ...
            <AssetDetailMlSignalPanel
              isMlSignalExpanded={isMlSignalExpanded}
              setIsMlSignalExpanded={setIsMlSignalExpanded}
              mlSignal={mlSignal}
              mlSignalLoading={mlSignalLoading}
              mlSignalMessage={mlSignalMessage}
              resolvedAssetType={resolvedAssetType}
              isResolvedUsStock={isResolvedUsStock}
              onFetchMlSignal={fetchMlSignal}
            />
