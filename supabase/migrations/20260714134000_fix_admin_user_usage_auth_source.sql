-- 관리자 유저 목록은 auth.users 전체를 기준으로 보이게 하고,
-- 과거 트리거 누락/실패로 빠진 profiles row를 백필한다.

INSERT INTO public.profiles (id, email, nickname, phone, updated_at)
SELECT
    u.id,
    u.email,
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'nickname', COALESCE(NULLIF(p.nickname, ''), u.raw_user_meta_data->>'nickname', ''),
        'role', COALESCE(p.role, 'USER'),
        'updatedAt', COALESCE(p.updated_at, u.updated_at, u.created_at)
    )
    INTO v_user
    FROM auth.users AS u
    LEFT JOIN public.profiles AS p ON p.id = u.id
