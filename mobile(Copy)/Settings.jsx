...
export default function Settings({ isLoggedIn, userEmail, handleLogout, userProfile, setUserProfile, hideHeader, mobileLayout = false }) {
  ...
  const secondaryButtonClass = 'w-full rounded border border-slate-700 bg-slate-800 px-3 py-2.5 text-xs font-bold text-white transition-all hover:bg-slate-700 disabled:opacity-50'

  return (
    <div className={`min-h-screen bg-obsidian-bg text-[#e2e2ec] font-inter ${mobileLayout ? 'px-3 py-4' : 'px-6 py-8'}`}>
      {/* 공통 상단 네비게이션 헤더 */}
      ...
      <main className={`max-w-4xl mx-auto flex flex-col ${mobileLayout ? 'mt-3 gap-5' : 'mt-6 gap-8'}`}>
        ...
        {/* 탭 기반 키 등록 설정 입력창 */}
        <section className={sectionClass}>
          <div className={`${mobileLayout ? 'grid grid-cols-2 gap-2' : 'flex gap-2'} border-b border-slate-800 pb-2`}>
            {['TOSS', 'KIS', 'COINONE', 'BINANCE'].map((tab) => (
              <button
                ...
                className={`rounded-lg px-4 py-2.5 text-xs font-extrabold tracking-widest transition-all cursor-pointer border ${activeTab === tab
        ...
        <section className={sectionClass}>
          ...
          <div className={`bg-[#0F172A] border border-ai-cyan/40 rounded-lg ${mobileLayout ? 'p-4' : 'p-6'}`}>
        ...
        <section className={sectionClass}>
          ...
          <form onSubmit={handleSaveProfile} className="flex flex-col gap-2">
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Nickname</label>
            ...
            <p className="text-[11px] leading-5 text-slate-500">2-16 characters, letters, numbers, and underscore only.</p>
          </form>
